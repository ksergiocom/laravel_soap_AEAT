...
class SoapService
{
    ...
    private VeriFactuHashService $hashService; // Necesario para generar los HASH
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $this->hashService = new VeriFactuHashService();
    ...
    public function altaFactura()
    {
        ...
        $numeroSerieFactura = '12345';

        $cuota = '210';
        $importe = '1210';
        $huellaAnterior = ''; // Primera en la cadena

        $fechaISO8601 = (new \DateTime())->format('c');

        $dto = new VeriFactuAltaDTO(
            IDEmisorFactura: $nif,
            NumSerieFactura: $numeroSerieFactura,
            FechaExpedicionFactura: $fechaExpedicion,
            TipoFactura: 'F1',
            CuotaTotal: $cuota,
            ImporteTotal: $importe,
            HuellaAnterior: $huellaAnterior,
            FechaHoraHusoGenRegistro: $fechaISO8601,
        );

        $huella = $this->hashService->generarHuellaAlta($dto);
        ...
        $registroFacturacionAlta = [
            ...
            'NombreRazonEmisor' => env('NOMBRE_RAZON', 'ValorPorDefecto'),
            'TipoFactura' => 'F1',
            'DescripcionOperacion' => 'Venta de algo',
            'Destinatarios' => [
                'IDDestinatario' => [
                    'NombreRazon' => '1970 Acuale S.L. S.L.',
                    'NIF' => 'B39846274',
                ],
            ],
            'Desglose' => [
        ...
        $params = [
            ...
            'RegistroFactura' => [
                'RegistroAlta' => $registroFacturacionAlta,
            ],
        ];

        $res = $this->client->RegFactuSistemaFacturacion($params);
